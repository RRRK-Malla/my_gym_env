...
class Car:
    def __init__(self, car_file, map_file, pos):
        self.layer = pygame.image.load(car_file)
        self.map = pygame.image.load(map_file)
        self.layer = pygame.transform.scale(self.layer, (100, 100))
        self.rotate_layer = self.layer
        self.pos = pos
        self.angle = 0
        self.speed = 0
    ...
    def draw(self, screen):
        screen.blit(self.rotate_layer, self.pos)
    ...
    def check_collision(self):
        self.on_game = True
        for y in self.four_points:
            if self.map.get_at((int(y[0]), int(y[1]))) == (255, 255, 255, 255):
                self.on_game = False
                break

    def check_radar(self, degree):
        len = 0
        m = int(self.center[0] + math.cos(math.radians(360 - (self.angle + degree))) * len)
        n = int(self.center[1] + math.sin(math.radians(360 - (self.angle + degree))) * len)
    ...
    def check_checkpoint(self):
        k = check_point[self.game_check]
        self.prev_distance = self.cur_distance
        dist = get_distance(k, self.center)
        if dist < 70:
            self.game_check += 1
            self.prev_distance = 9999
            self.check_flag = True
            if self.game_check >= len(check_point):
                self.game_check = 0
                self.goal = True
            else:
                self.goal = False
    ...
    def update(self):
        ...
        #check position
        self.rotate_layer = center_rotation(self.layer, self.angle)
...
class driveGame:
    ...
    def disp(self):
        ...
        pygame.draw.circle(self.screen, (255, 255, 0), check_point[self.car.game_check], 70, 1)
...
def center_rotation(image, angle):
